...
def calculaCaminho(grafo, origem, fim, carga):
    distanciaAtual = dijkstra(grafo, origem, fim)
    carga_necessaria = distanciaAtual[fim] - carga
    ...
    if(dijkstra_path(grafo, origem, fim)[0] < carga):
        ...
        while dijkstra_path(grafo, origem, fim)[0] > carga:
            inicio_aux = ""
            for vertices in distanciaAtual:
                if "R" in vertices and dijkstra_path(grafo, origem, vertices)[0] < carga:
                    if maior_custo_pre_recarga < dijkstra_path(grafo, origem, vertices)[0]:
                        ...
                        calculated_path = str(dijkstra_path(grafo, origem, vertices)[1]) + " Recarga "
                        gasto_total = dijkstra_path(grafo, origem, vertices)[0]
                        inicio_aux = str(vertices)
            print(calculated_path)

            if dijkstra_path(grafo, inicio_aux, fim)[0] < carga:
                ...
                distanciaAtual2 = dijkstra(grafo, inicio_aux, fim)
                calculated_path += " " + str(dijkstra_path(grafo, inicio_aux, fim)[1])
                distanciaAtual = {}

                for vertice, j in distanciaAtual2.items():
                    if j != float('inf'):
                        distanciaAtual[vertice] = j
                grafo2 = {}

                for vertice, peso in grafo.items():
                    if vertice in distanciaAtual.keys():
                        grafo2[vertice] = peso
                grafo = grafo2.copy()
                origem = inicio_aux

calculaCaminho(grafo3, "A", "Ii", 150)
